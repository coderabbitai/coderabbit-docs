...
<ListItems orderedList items={[
  <><strong>Navigate to Webhook Settings</strong>: Go to the repository settings and locate the webhooks configuration page.</>,
  <><strong>Configure Events</strong>: Enable the following Pull Request events:
  ...
  <><strong>Add Webhook URL</strong>: Enter the URL pointing to the CodeRabbit service, followed by <code>/bitbucket_server_webhooks</code> (e.g., <code>http://127.0.0.1:8080/bitbucket_server_webhooks</code>).</>
]} />
