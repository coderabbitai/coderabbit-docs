...
<DarkCodeBlock language="bash" title=".env">{`# if using OpenAI
LLM_PROVIDER=openai
LLM_TIMEOUT=360000
...
YAML_CONFIG=[<escaped-yaml-config>]`}</DarkCodeBlock>
...
<DarkCodeBlock language="bash">{`cat coderabbit.json | docker login -u _json_key --password-stdin us-docker.pkg.dev
docker pull <docker-registry>/coderabbit-agent:latest`}</DarkCodeBlock>
...
<DarkCodeBlock language="bash">{`docker run --env-file .env --publish 127.0.0.1:8080:8080 <docker-registry>/coderabbit-agent:latest`}</DarkCodeBlock>

### Verify the image is up

You can query `/health` endpoint to verify that the `coderabbit-agent` service is up and running.

<DarkCodeBlock language="bash">{`curl 127.0.0.1:8080/health`}</DarkCodeBlock>
